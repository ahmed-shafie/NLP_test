...
- The write is guarded by a **unique account** constraint in Banking Core, so re-running a completed
  ...
  (status still shows `completed`). Check `banking-core/banking_core.db` (or the container's DB) first and
  pick a fresh name+IBAN pair if you want to prove a real write.
