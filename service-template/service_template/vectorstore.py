...
Mirrors ``app/vectorstore.py``. Vectors are assumed to be L2-normalised, so the
inner product (``IndexFlatIP``) equals cosine similarity. Payloads (arbitrary
objects — here ``(text, intent)`` tuples) are kept in a parallel list and
returned alongside similarity scores at search time.
"""
